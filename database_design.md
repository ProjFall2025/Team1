...
**Support:** Events (CRUD + organizer_id), Bookings (per event), Payments (refunds).
...
- **Organizer linkage** is established through `Events.organizer_id`.  
- **Bookings** enforce one booking per user/event using the unique constraint `UNIQUE(user_id, event_id)`.  
